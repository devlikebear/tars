...
| Work | Chat | `/console/chat` | Interactive agent chat with tool calling, dockable Sessions and tool panels, task contract review/approval, task evidence attachments, read-only Git Inspector, message-level session forking, `@` file/directory/subagent mentions, `/` command popover for client commands and skills, transcript-snippet session search, parallel subagent progress cards, Files workspace shell, Prior Context preview, and advanced `/config` session policy and automation consent overrides |
...
| Operate | Approvals | `/console/approvals` | Review risky cleanup plans before TARS applies them and inspect the Automation Audit log |
