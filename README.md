...
- **Per-project overrides (`.tars/`)** — Drop a `.tars/settings.json` (team-shared, commit it) or `.tars/settings.local.json` (per-user, gitignore it) under the chat session's active working directory and TARS will fold those values into the session's `tool_config` / `prompt_override` for chats run in that directory. A sibling `.tars/skills/<name>/SKILL.md` adds project-only skills, and `.tars/commands/<name>.md` registers thin slash-command aliases over existing skills via a `target_skill:` frontmatter key. The active cwd is shown as an amber chip in the chat header (or via `/cwd`); merged values render with a `shared` / `local` source badge in the Session Config panel. The whitelist is narrow on purpose — credentials, hooks, and arbitrary binary paths are blocked at load time. See `CLAUDE.md` for the full schema.
