<script lang="ts">
  ...
  function promptImpactLine(file: SyspromptFile): string {
    ...
    if (!impact) return ''
    return $t.sysprompt.impactLine(
      formatBytes(file.size_bytes || 0),
      impact.estimated_tokens,
      impact.chars,
      impact.max_chars,
      impact.section,
      impact.role,
  ...
  function promptImpactWarning(file: SyspromptFile): string {
    ...
    return $t.sysprompt.truncateWarning(impact.chars, impact.max_chars)
  }
  ...
  async function load(targetScope?: SyspromptScope, targetPath?: string) {
    ...
    try {
      ...
      error = err instanceof Error ? err.message : $t.sysprompt.failedLoadFiles
    } finally {
      loading = false
    }
  }
  ...
  async function selectFile(scope: SyspromptScope, path: string) {
    ...
    try {
      ...
      error = err instanceof Error ? err.message : $t.sysprompt.failedLoadFile
    }
  }
  ...
  async function save() {
    ...
    try {
      ...
      success = $t.sysprompt.savedSuccess(saved.path)
    } catch (err) {
      error = err instanceof Error ? err.message : $t.sysprompt.failedSave
    } finally {
      saving = false
    }
  }
  ...
  async function loadPreview() {
    ...
    try {
      ...
      previewError = err instanceof Error ? err.message : $t.sysprompt.failedLoadPreview
    } finally {
  ...
  function handleTemplateInsert() {
    ...
    selectedTemplateId = ''
    error = ''
    success = $t.sysprompt.templateInsertedSuccess(template.label)
  }

  function roleCopy(path: string): string {
    switch (path) {
      case 'USER.md':
        return $t.sysprompt.role.USER
      case 'IDENTITY.md':
        return $t.sysprompt.role.IDENTITY
      case 'AGENTS.md':
        return $t.sysprompt.role.AGENTS
      case 'TOOLS.md':
        return $t.sysprompt.role.TOOLS
      case 'PROJECT.md':
        return $t.sysprompt.role.PROJECT
      default:
        return $t.sysprompt.role.default
    }
  }
...
<div class="sysprompt-page">
  <header class="hero card">
    <div>
      <div class="eyebrow">{$t.sysprompt.eyebrow}</div>
      <h1>{$t.sysprompt.title}</h1>
      <p>{$t.sysprompt.subtitle}</p>
    </div>
    <div class="hero-stats">
      <div class="stat">
        <span class="stat-label">{$t.sysprompt.statWorkspaceFiles}</span>
        <strong>{workspaceFiles.length}</strong>
      </div>
      <div class="stat">
        <span class="stat-label">{$t.sysprompt.statAgentFiles}</span>
        <strong>{agentFiles.length}</strong>
      </div>
      <div class="stat">
        <span class="stat-label">{$t.sysprompt.statBuiltinTools}</span>
  ...
  <div class="layout" class:technical-open={showTechnicalDetails}>
    <aside class="card file-panel">
      <div class="panel-header">
        <span class="card-title">{$t.sysprompt.workspacePromptTitle}</span>
      </div>
      {#if loading}
        <div class="empty-state">{$t.sysprompt.loadingFiles}</div>
      {:else}
        <div class="file-group">
          <div class="group-label">{$t.sysprompt.groupWorkspaceIdentity}</div>
            ...
            <button class="file-row" class:active={selectedScope === file.scope && selectedPath === file.path} type="button" onclick={() => selectFile(file.scope, file.path)}>
              <div class="file-row-top">
                ...
                <span class:badge-success={file.exists} class:badge-warning={!file.exists} class="badge">
                  {file.exists ? $t.sysprompt.badgePresent : $t.sysprompt.badgeMissing}
                </span>
              </div>
        ...
        <div class="file-group">
          <div class="group-label">{$t.sysprompt.groupAgentRules}</div>
    ...
    <section class="card editor-panel">
      <div class="panel-header">
        <div>
          <span class="card-title">{$t.sysprompt.editorTitle}</span>
          <div class="panel-subtitle">{selectedFile ? `${selectedFile.scope} · ${selectedFile.path}` : $t.sysprompt.selectFilePrompt}</div>
        </div>
        <div class="editor-actions">
          <button class="btn btn-ghost btn-sm" type="button" disabled={!selectedFile} onclick={() => selectedFile && selectFile(selectedFile.scope, selectedFile.path)}>{$t.sysprompt.reload}</button>
          <button class="btn btn-ghost btn-sm" type="button" disabled={!selectedFile || previewLoading} onclick={loadPreview}>
            {previewLoading ? $t.sysprompt.previewLoading : $t.sysprompt.reloadPreview}
          </button>
          <button
          ...
          >
            {showTechnicalDetails ? $t.sysprompt.hideTechnicalDetails : $t.sysprompt.showTechnicalDetails}
          </button>
          <button class="btn btn-primary btn-sm" type="button" disabled={!selectedFile || saving} onclick={save}>
            {saving ? $t.sysprompt.saving : $t.sysprompt.save}
          </button>
        </div>
      </div>
      ...
      {#if selectedFile}
        <div class="meta-row">
          <span class="badge badge-accent">{selectedFile.scope === 'workspace' ? $t.sysprompt.badgeMainAgent : $t.sysprompt.badgeSubAgent}</span>
          <span>{selectedFile.exists ? $t.sysprompt.existingFile : $t.sysprompt.missingFileStarter}</span>
          <span>{formatBytes(selectedFile.size_bytes || 0)}</span>
          <span>{fmt(selectedFile.updated_at)}</span>
          {#if selectedFile.prompt_impact}
            <span>{$t.sysprompt.tokensSuffix(selectedFile.prompt_impact.estimated_tokens)}</span>
            <span>{$t.sysprompt.sectionLabel(selectedFile.prompt_impact.section, selectedFile.prompt_impact.role)}</span>
          {/if}
        </div>
        ...
        {#if canInsertTemplate}
          <div class="starter-template-bar">
            <div>
              <strong>{$t.sysprompt.insertTemplateTitle}</strong>
              <p>{selectedTemplate?.description || $t.sysprompt.insertTemplateDefaultDesc}</p>
            </div>
            <div class="starter-template-actions">
              <select class="template-select" bind:value={selectedTemplateId} aria-label={$t.sysprompt.starterAriaLabel}>
                <option value="" disabled>{$t.sysprompt.chooseTemplatePlaceholder}</option>
      ...
      {:else}
        <div class="empty-state">{$t.sysprompt.selectFileEmpty}</div>
      {/if}
    </section>
    ...
    {#if showTechnicalDetails}
      <aside class="card diagnostics-panel">
        <div class="panel-header">
          <span class="card-title">{$t.sysprompt.diagnosticsTitle}</span>
        </div>

        <div class="diag-block">
          <div class="group-label">{$t.sysprompt.roleSemanticsLabel}</div>
          <div class="diag-list">
            <div><strong>USER.md</strong><span>{$t.sysprompt.diagRoles.USER}</span></div>
            <div><strong>IDENTITY.md</strong><span>{$t.sysprompt.diagRoles.IDENTITY}</span></div>
            <div><strong>PROJECT.md</strong><span>{$t.sysprompt.diagRoles.PROJECT}</span></div>
            <div><strong>AGENTS.md</strong><span>{$t.sysprompt.diagRoles.AGENTS}</span></div>
            <div><strong>TOOLS.md</strong><span>{$t.sysprompt.diagRoles.TOOLS}</span></div>
          </div>
        </div>

        <div class="diag-block">
          <div class="group-label">{$t.sysprompt.relevantToolsLabel}</div>
          {#if relevantTools.length === 0}
            <div class="empty-state">{$t.sysprompt.noToolsDetected}</div>
          {:else}
            <div class="tool-list">
  ...
  {#if previewOpen}
    <div class="preview-backdrop" role="presentation">
      <div class="preview-modal" role="dialog" aria-modal="true" aria-label={$t.sysprompt.previewModalLabel} tabindex="0">
        <div class="preview-header">
          <div>
            <div class="group-label">{$t.sysprompt.previewKicker}</div>
            <h2>{preview?.target === 'sub_agent' ? $t.sysprompt.previewSubTitle : $t.sysprompt.previewMainTitle}</h2>
          </div>
          <div class="preview-actions">
            <button class="btn btn-ghost btn-sm" type="button" disabled={previewLoading} onclick={loadPreview}>
              {previewLoading ? $t.sysprompt.previewLoading : $t.sysprompt.reloadPreview}
            </button>
            <button class="btn btn-ghost btn-sm" type="button" onclick={() => { previewOpen = false }}>{$t.sysprompt.previewClose}</button>
          </div>
        </div>
        {#if previewError}
          <div class="error-banner">{previewError}</div>
        {:else if previewLoading && !preview}
          <div class="empty-state">{$t.sysprompt.previewLoadingState}</div>
        {:else if preview}
          <div class="preview-meta">
            <span class="badge badge-accent">{preview.target}</span>
            <span>{$t.sysprompt.tokensTotal(preview.total_tokens)}</span>
            <span>{$t.sysprompt.tokensStatic(preview.static_tokens)}</span>
            <span>{$t.sysprompt.tokensMemory(preview.relevant_tokens)}</span>
          </div>
...
<style>
  ...
  .sysprompt-page > .hero,
  .sysprompt-page > .error-banner,
  .sysprompt-page > .success-banner {
    flex-shrink: 0;
  }

  .sysprompt-page > .layout {
    flex: 1;
    min-height: 0;
  }
