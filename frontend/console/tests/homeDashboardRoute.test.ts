...
test('Home dashboard exposes system status, sessions, notifications, and actions', () => {
  ...
  assert.match(homeSource, /Mission Control/)
  assert.match(homeSource, /Active plans/)
  assert.match(homeSource, /Agent runs/)
  assert.match(homeSource, /Cron jobs/)
  assert.match(homeSource, /Delivery/)
