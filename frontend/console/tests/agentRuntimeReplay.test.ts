...
test('deriveAgentRuntimeReplayState applies only events up to the cursor', () => {
  const state = deriveAgentRuntimeReplayState(events, Date.parse('2026-05-01T00:00:05Z'), labels)
...
test('deriveAgentRuntimeReplayState labels the empty cursor with the given words', () => {
  const state = deriveAgentRuntimeReplayState(events, Date.parse('2026-04-30T23:59:59Z'), labels)

  assert.equal(state.appliedCount, 0)
  assert.equal(state.lastEventType, 'none')
})
