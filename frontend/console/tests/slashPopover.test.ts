...
test('SlashPopover renders command and skill candidates with active state', () => {
  ...
  assert.match(popoverSource, /candidate\.kind === 'builtin' \? 'Built-in' : 'Skills'/)
...
test('Chat executes client-side slash commands without sending to the LLM', () => {
  ...
  assert.match(chatSource, /getSessionConfig\(selectedSessionId\)/)
  assert.match(chatSource, /updateSessionConfig\(selectedSessionId, nextConfig\)/)
