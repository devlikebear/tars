...
test('Pulse introduces the system watchdog scope before status readouts', () => {
  assert.match(source, /Pulse - System Watchdog/)
  assert.match(source, /pulseWatchItems/)
  assert.match(source, /Cron job failures/)
  assert.match(source, /Agent runtime stuck runs/)
  assert.match(source, /Disk pressure/)
  assert.match(source, /Telegram delivery failures/)
  assert.match(source, /Reflection nightly failures/)
  assert.match(source, /LLM classifier/)
  assert.match(source, /ignore/)
  assert.match(source, /notify/)
  assert.match(source, /autofix/)
  assert.match(source, /Settings -> pulse_\*/)
