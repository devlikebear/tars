...
- **콘솔 표면 축소 및 #919 Phase 2-3 / #930 동결 (#931)** — 내비게이션을 chat + pulse, 그리고 남은 페이지가 동작하는 데 필요한 approvals / logs / config만 남기고 정리했다. lineage, plans, memory, sysprompt, extensions, agentruntime, channels, cron, analytics, reflection은 **nav에서만 감춘 것**이라 라우트·컴포넌트·백엔드 패키지는 그대로이고 URL로는 여전히 접근된다 — 실제로 매일 쓰는 것보다 훨씬 넓은 표면을 계속 유지·보수하는 비용이 문제였지 코드가 잘못된 게 아니었다. `/console/config`는 Quick Start 점검만 남기고 165필드 Inspect 패널과 YAML 읽기 뷰를 제거했다(read-only가 아니라 제거). Quick Start 카드, 온보딩 마법사 재진입, Test connection 딥링크, Remote Access, 재시작, 워크스페이스 리셋은 그대로다. `internal/config` 스키마와 설정 쓰기 라우트는 손대지 않았으므로 CLI/스크립트 경로는 영향이 없다. 아울러 epic #919의 Phase 2(public library boundary, #927/#928/#929)와 Phase 3의 #930(레이어 경계 강제 + 멀티레포 분리 유예)은 **동결**한다 — ADR·depguard 규칙·패키지 이동은 이번에 구현하지 않으며, #931 LP-012는 이 축소로 마무리된 것으로 본다. 정책 본문은 `frontend/console/DESIGN.md`의 Console Purpose & Surface Policy에 기록했다.

### Fixed

- **`openai-codex` 모델 목록이 항상 401이었고, 그 401이 멀쩡한 리프레시 토큰을 소모하던 문제** — `FetchModels`가 Codex에 대해 `https://api.openai.com/v1/models`, 즉 OpenAI **플랫폼** API를 쳤다. ChatGPT OAuth 토큰은 플랫폼 API 키가 아니므로 그 호스트는 매번 401을 돌려줬고, 코드는 그 401을 "토큰 만료"로 읽어 `refreshCredential(PersistSource: true)`로 유효한 리프레시 토큰을 쓰고 `auth.json`을 다시 쓴 뒤 같은 잘못된 호스트로 재시도했다 — 로그인이 정상인 사용자에게 "다시 로그인하라"는 안내가 나가던 경로다. 채팅은 처음부터 `llmdefaults.OpenAICodexBaseURL`(ChatGPT 백엔드)로 가고 있었다. 이제 Codex CLI와 같은 곳(`<base>/codex/models?client_version=…`, `codex-rs/codex-api/src/endpoint/models.rs`)을 채팅과 같은 세 헤더(`Authorization`, `chatgpt-account-id`, `originator`)로 호출하고, 응답도 OpenAI 스타일 `{data:[{id}]}`가 아니라 백엔드의 `{models:[{slug, visibility, priority, …}]}`로 읽는다. `visibility: "list"`인 슬러그만 백엔드의 priority 순으로 돌려주며(`hide`는 `codex-auto-review` 같은 내부 모델), `client_version`에는 CLI 버전을 흉내내지 않고 tars 자신의 빌드 버전을 보낸다. 401 시 리프레시-1회-재시도는 그대로지만 이제 올바른 호스트에서만 일어나므로 실제로 만료된 토큰에만 닿는다.

- **Codex 채팅이 매번 왕복 하나를 버리던 문제** — ChatGPT 백엔드는 스트리밍이 아닌 요청에 400 `"Stream must be set to true"`을 돌려준다. 클라이언트는 `OnDelta`가 없으면 `stream:false`로 먼저 보내고 그 400을 받은 뒤에야 `stream:true`로 다시 보냈으므로, `Ask()`와 델타 콜백 없는 모든 `Chat()` — 연결 테스트가 정확히 이 경우다 — 가 첫 응답 전에 요청 하나를 낭비했다. Codex는 스트림 전용이므로 처음부터 `stream:true`, `Accept: text/event-stream`으로 보내고, JSON 응답 파서와 "stream required" 폴백 경로는 삭제했다. SSE 파서는 `OnDelta`가 없어도 완전한 `ChatResponse`를 조립하므로 호출자 쪽 차이는 없다. 백엔드가 스트리밍 요청에 그 400을 돌려주는 일이 생기면 이제 조용히 재전송하지 않고 오류로 올린다.

- **`llmdefaults.OpenAICodexModel`이 더 이상 서비스되지 않는 `gpt-5.3-codex`를 가리키던 문제** — ChatGPT 백엔드는 슬러그를 예고 없이 내리며, 이 값으로 보낸 요청은 400 `"not supported when using Codex with a ChatGPT account"`을 받았다. 모델을 비워 둔 호출자는 이 상수를 쓰므로 "기본값"이 곧 실패였다. 2026-09-08 기준 실제로 서비스되는 `gpt-5.6-sol`로 바꿨다. 모델 목록 엔드포인트가 진실의 원천이니, 목록을 받을 수 있는 호출자는 이 상수보다 목록을 우선해야 한다.

- **타임아웃된 검증 명령이 서술자를 남기면 `proofverifier`가 영구히 매달리던 문제** — `processCommandRunner.Run`은 검증 명령을 `sh -lc`로 실행하면서 stdout/stderr를 `strings.Builder`에 받는다. `*os.File`이 아니므로 `os/exec`가 파이프와 복사 고루틴을 만들고, `Wait`는 파이프의 **모든** 쓰기 끝이 닫힐 때까지 블록한다. 명령이 백그라운드 서술자를 띄웠다면 셸만 죽여도 그 서술자가 파이프를 계속 붙들기 때문에, 타임아웃이 걸린 뒤에도 `Run`이 데드라인에 반환하지 않고 서술자 수명만큼 매달렸다 — 임의의 셸 문자열을 실행하는 검증기에서는 스케줄러 고루틴이 무기한 묶이는 결과가 된다. `internal/llm`의 CLI 프로바이더들이 이미 쓰던 방식을 그대로 따라, unix에서는 셸을 자체 프로세스 그룹으로 띄우고 취소 시 그룹 전체를 SIGKILL하며, 양쪽 플랫폼 모두 `cmd.WaitDelay`(5초)로 잔여 파이프 대기를 끊는다. Windows에는 POSIX 프로세스 그룹이 없어 `WaitDelay`가 유일한 방어선이다. 아울러 이 경로를 밟게 만든 테스트 쪽 1초 예산을 30초로 올렸다 — 해당 호출들은 타이밍을 검증하지 않는데, 느린 Windows 러너에서는 Git Bash 로그인 셸이 프로필을 읽는 데만 1초 가까이 써서 CI가 산발적으로 깨졌다.

- **Anthropic에서 `reasoning_effort`가 무시되던 문제 (#922)** — Anthropic 클라이언트는 `effectiveThinkingBudget`만 읽고 `ReasoningEffort`는 전혀 보지 않았다. 출하되는 `config/default.yaml`이 이미 `kind: anthropic` tier(`light`)에 `reasoning_effort: minimal`을 걸어두고 있으므로, 검증되고 콘솔에 표시되며 아무 일도 하지 않는 설정이 그대로 배포되고 있었다. Messages API에는 네이티브 effort 노브가 없어 effort 레벨을 extended thinking 예산으로 번역한다 — `none` 끄기 / `minimal` 1024 / `low` 2048 / `medium` 8192 / `high` 16384. 예산은 답변 몫을 남기도록 `max_tokens`에서 headroom(1024)을 뺀 값으로 클램프되고, 그마저 프로바이더 하한(1024)에 못 미치면 거부당할 요청을 보내는 대신 thinking을 끄고 경고를 남긴다. `thinking_budget`은 더 구체적인 노브이므로 여전히 `reasoning_effort`보다 우선한다. 매핑표는 `config/tars.config.example.yaml`에 정리했다. 부수 효과로, 예산과 `max_tokens`가 같은 값이 되던(프로바이더가 거부하는 조합) 기존 동작도 함께 고쳐진다.

- **Anthropic assistant 턴에서 thinking 블록이 유실되던 문제 (#922)** — `toAnthropicAssistantMessage`가 assistant 턴을 text + `tool_use`로만 재구성했고, `ContentBlock`에 thinking/signature 필드가 없어 파싱된 추론은 문자열 `ReasoningContent`로 납작해진 뒤 사라졌다. Anthropic은 extended thinking이 켜진 상태에서 `tool_use`를 실은 assistant 턴에는 서명된 thinking 블록을 그대로 돌려받아야 하므로, thinking과 도구를 함께 쓰는 tier는 tool 루프 2회차부터 실패하게 되어 있었다 — 출하 tier 중 `thinking_budget`을 켠 것이 없어 잠재 상태였을 뿐이다. `gemini-native`가 `thoughtSignature`를 왕복시키는 방식(`gemini_native_convert.go`)을 그대로 따라, `ChatMessage.ReasoningBlocks`에 블록을 순서대로 보존하고 스트리밍·논스트리밍 파서 양쪽에서 서명(`signature_delta` 포함)을 수집한 뒤 `tool_use`보다 앞서 원래 순서로 다시 내보낸다. `redacted_thinking`은 내용을 들여다보지 않고 불투명하게 왕복한다. 서명이 없는 thinking 블록(이 변경 이전에 기록된 transcript)은 거부될 요청을 보내는 대신 조용히 빠진다. 블록은 transcript(`session.Message.reasoning_blocks`)에도 저장되므로 대화 도중 서버가 재시작해도 복원된다.
...
- **Crash-safe remote scheduler binding (#903)** — Connects an explicitly configured in-process container or SSH worker to the durable scheduler, journals the bounded workspace before execution and the verified result before placement completion, replays committed worker results with fresh authorization after gateway loss, and deletes recovery state only after the Work Ledger Attempt commits and cleanup succeeds.
- **Remote execution security boundary (#903)** — Adds short-lived Ed25519 task tokens bound to Work/Step/Attempt/placement/worker and scope, Git-aware and directory sync manifests, default-deny container egress, hard resource and wire limits, artifact quarantine, secret redaction, and non-canonical token rejection without putting provider credentials in worker files or environments.
- **A2A 1.0 scheduler adapter (#903)** — Adds strict Agent Card discovery and HTTP+JSON message/task operations, restart-safe external task journaling, cancellation, operator-action states, bounded output sanitization, same-host/allowlist and private-network controls, and quarantines URL/file/raw artifact parts. A2A remains an explicit external-agent adapter rather than an MCP or internal-scheduler replacement.
- **Remote Execution Console (#903)** — Adds a sanitized operator view for worker health, placements, sync size, resource and egress policy, checkpoints, recovery counts, A2A activation, and recent state transitions, plus operator-friendly worker/A2A labels in the durable Tasks timeline.
- **Reviewed self-improvement lifecycle (#908)** — Replaces Skill Inbox's immediate active-skill write with versioned candidate, draft, sandbox, offline-eval, shadow, human-approval, canary, promotion, rejection, and one-action rollback states. CapabilityVersion, EvaluationRun, independent Proof, permission delta, rollout, and later Work outcome records are durable in the Work Ledger and visible in the Console.
- **Capability regression review and non-destructive migration (#908)** — Imports the existing Skill Inbox into the ledger without rewriting its JSONL source, records repeated success/failure/dead-end/user-correction signals, exports and diagnoses capability records, and flags failed post-promotion outcomes for review and rollback.
