...
curl -sS \
  ...
  | jq -e '.authenticated == true and .auth_role == "user" and (has("workspace_id") | not)' >/dev/null

echo "[2/5] whoami (admin)"
curl -sS \
  -H "Authorization: Bearer ${ADMIN_TOKEN}" \
  "${SERVER_URL}/v1/auth/whoami" \
  | jq -e '.authenticated == true and .auth_role == "admin" and .is_admin == true and (has("workspace_id") | not)' >/dev/null
...
curl -sS \
  ...
  | jq -e '(has("workspace_id") | not) and (.workspace_dir | type == "string")' >/dev/null
