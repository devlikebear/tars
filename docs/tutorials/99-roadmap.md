...
Phase 5 📦  채팅 인터페이스 (프로젝트/대시보드는 제거됨)
Phase 6 📦  자율 실행 (Autopilot/대시보드는 제거됨 → pulse/reflection으로 대체)
Phase 7 📦  외부 경계와 배포 (TUI 제거됨 → 웹 콘솔로 대체)
```
...
> **Note:** 프로젝트 Store, Kanban, macOS 대시보드(Steps 16-17)는 제거되었습니다.
> 현재 지원 surface는 웹 콘솔(`/console`)과 CLI입니다.
...
> **Note:** Autopilot, 대시보드 감독 UI(Steps 18-20)는 제거되었습니다.
> 시스템 자동화는 pulse(watchdog) + reflection(nightly batch)으로 대체되었습니다.
