...
이 모듈은 HTTP 요청이 어떤 권한으로 실행되는지 결정하고, 외부 확장인 plugin이 어떤 skill/MCP를 노출할 수 있는지 안전하게 정리하는 경계 계층이다.
...
`internal/plugin/loader.go`는 plugin manifest를 모두 읽어 snapshot으로 합친다.
