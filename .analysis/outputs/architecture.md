...
- 허브 관리 경로: `cmd/tars/skill_main.go`, `cmd/tars/plugin_main.go`
- 메인 스레드 shim: `cmd/tars/mainthread_darwin.go`, `cmd/tars/mainthread_other.go`

핵심 포인트는 엔트리포인트가 계속 얇다는 점이다. 새 명령이 늘어났어도 실제 로직은 `internal/*`로 바로 내려간다.
...
- 세션, 프로젝트, 사용량, cron, schedule, gateway, browser, events, dashboard, provider model 핸들러를 하나의 mux 아래 둔다.
...
`dashboard_auth_mode=off`는 이제 완전 공개가 아니라 "loopback 요청만 skip"으로 바뀌었다. 즉, 공개 모드의 위험을 줄이되 운영상 편의는 남겨 둔 형태다.

## 5. 채팅 실행 레이어

채팅 파이프라인은 네 파일과 두 helper를 같이 봐야 한다.
...
추가로 이 레이어는 프로젝트 workflow 진입도 맡는다. kickoff 문장이나 활성 brief가 있으면 `handler_chat.go`가 `project-start` skill을 자동 선택한다.
...
skill mirror가 새로 중요해진 이유는 agent가 source path가 아니라 workspace 내부의 안정된 runtime path를 읽기 때문이다. companion file도 같이 복사되므로 skill이 참조하는 script/config를 runtime 경로에서 그대로 찾을 수 있다.
...
- `workflow_policy.go`: brief/project status normalize, 기본 next action, dispatch stage, blocked/done state 전이 규칙을 모은다.
- `orchestrator.go`: todo/review dispatch, verification gate, agent report 기록을 처리한다.
- `project_runner.go`: autopilot loop를 돌며 backlog seed, auto-retry, blocked/done 전이를 관리한다.
- `task_report.go`: worker output을 고정 포맷 `<task-report>`로 파싱한다.

핵심 포인트는 "정책이 조금 더 분리됐지만 아직 chat trigger, dispatch, supervision, dashboard projection이 완전히 한 모델로 합쳐지지는 않았다"는 점이다.
...
이제 dashboard 섹션은 `projectDashboardSectionRegistry`로 중앙화되어 있다. 서버 렌더, refresh 대상 ID, section별 추가 계산이 한 registry에서 같이 정의된다.
...
즉, 비동기 런타임은 이제 "run lifecycle + channel surface + report surface"가 더 분리돼 있다.
