...
- 필요하면 project board/activity/dashboard를 나중에 얹을 확장 지점을 미리 파악한다.
- 나중에 provider adapter나 host integration 을 붙일 때 경계를 어디에 두어야 하는지 미리 파악한다.
...
```text
CLI -> HTTP chat endpoint -> LLM stub -> optional tool -> transcript save -> stream back
```
...
### Step 5. 원본 저장소처럼 점진 확장한다

작업 순서:

1. project policy를 추가하려면 `internal/project/store.go`를 참고한다.
...
### Step 5.5. provider adapter 는 마지막에 붙인다
...
### Step 6. 프로젝트 운영 레이어를 별도 확장으로 붙인다
