...
test('parseArgs reads config file', () => {
	...
	fs.writeFileSync(configPath, ['server_url: http://localhost:19090', 'session_id: sess-from-file', 'verbose: true', ''].join('\n'));
...
test('parseArgs gives cli flags precedence over config file', () => {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tars-ui-config-'));
	const configPath = path.join(dir, 'config.yaml');
	fs.writeFileSync(configPath, ['server_url: http://localhost:19090', 'session_id: sess-from-file', 'verbose: false', ''].join('\n'));
