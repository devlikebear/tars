...
func TestFinalizeRunLocked_PopulatesPolicyFailureMetadata(t *testing.T) {
	...
	rt.mu.Lock()
	rt.finalizeRunLocked(state, "", PromptExecutionMetadata{}, fmt.Errorf("tool not injected for this request: exec"), nil)
	rt.mu.Unlock()
...
func TestFinalizeRunLocked_HiddenWorkerSessionAppendsSummaryToMain(t *testing.T) {
	...
	rt.mu.Lock()
	rt.finalizeRunLocked(state, "drafted episode 3 outline and updated state", PromptExecutionMetadata{}, nil, nil)
	rt.mu.Unlock()
